...
public class MyTriggersActivity extends BaseUserActivity {
    private static final String TAG = MyTriggersActivity.class.getSimpleName();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        Log.d(TAG, "MyTriggersActivity");
        mLocalTextView.setText(R.string.title_activity_mytriggers);
    }

    protected void showTable() {
        String userid = uData.userid;
        String url = "http://www.shanvishield.com/safety/safety.php?go=getMyTriggers&pid=" + userid;
        processTableRequest(url, "MyTriggers");
    }

    protected void onTableResponse(JSONArray table, String type) {
        try {
            if (type.equals("MyTriggers")) {
                tableRows = new ArrayList<TableRow>();
                for (int i = 0; i < table.length(); i++) {
                    JSONArray arr = table.getJSONArray(i);
                    TableRow row = TableRow.parseMyTriggers(arr);
                    tableRows.add(row);
                }
    ...
    public void onClick(TableRow tableRow, TableAdapter tableAdapter) {
        ...
        final Dialog dialog=new Dialog(MyTriggersActivity.this);
